...
**具体问题描述**：
通过分析`AdminGoodsController`中的商品查询接口，发现以下问题：

1. **参数数量过多**：`list()`方法接收8个独立参数（goodsId, goodsSn, name, page, limit, sort, order），违反了方法参数不宜过多的设计原则
2. **参数类型混杂**：包含基本类型（Integer, String）和分页排序参数，缺乏统一的数据结构
3. **扩展性差**：新增查询条件时需要修改方法签名，影响现有调用方
4. **可读性差**：长参数列表降低了代码的可读性和维护性
5. **验证困难**：分散的参数难以进行统一的验证和错误处理
...
| 指标 | 优化前 | 优化后 | 改进程度 |
|------|--------|--------|----------|
| 参数数量 | 8个独立参数 | 1个对象参数 | 显著减少 |
| 扩展性 | 差（需修改方法签名） | 好（只需添加字段） | 显著提升 |
| 可读性 | 差（长参数列表） | 好（语义化对象） | 显著提升 |
| 验证能力 | 分散验证 | 统一验证 | 显著提升 |
| 维护成本 | 高 | 低 | 显著降低 |
